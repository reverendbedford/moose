...
# quarter-symmetry, LARGE DEFORMATION.  Force-controlled variant of
# examples/rigid/ld-inelastic/hertz_inelastic_finite_3d.i:
#
#   * A Scalar variable `rigid_offset` is the sphere's translation along
#     load_direction = (0, 1, 0) (upward, into the material).
#   * A RigidBodyLoadControl scalar kernel enforces
#         F(t) = spring*s + Sum_i w_i * lambda_i * (n_i . load_direction)
#     where the small stabilization spring keeps the scalar column of the
#     Jacobian well-conditioned during the initial contact transient (see
#     RigidBodyLoadControl.md).
#   * F(t) is ramped so peak indentation is comparable to the
#     displacement-controlled version's u_top = -0.1 (~s = 0.05 with the
#     spring absorbing ~10 % of the applied force at peak).
#   * The material's top surface is now pinned; the rigid body carries the
#     entire applied load.
#
...
[Functions]
  [applied_force]
    ...
    # Ramp to a peak force chosen so peak indentation is roughly comparable
    # to the displacement-controlled case (u_top = -0.1).  The Hertz-scale
    # peak reaction is p_0 * pi * a^2 ~ (E * a / R) * a^2 ~ E * a^3 / R;
    # at a ~ 0.5 and E ~ 1.4e7 that is ~2e5.  With ~10 % of the load
    # absorbed by the stabilization spring, apply ~2.5e5 total.
    x = '0 1'
    y = '0 2.5e5'
  []
[]
...
[ScalarKernels]
  [load_control]
    ...
    displacements = 'disp_x disp_y disp_z'
    c = 1.0
    # See load_controlled_elastic test for a discussion of the physical
    # spring stabilizer.
...
[BCs]
  ...
  []
  [top_pin]
    # Force-controlled: material top is held in place; the rigid body
    # translates under the applied force.
    type = DirichletBC
    variable = disp_y
    boundary = 2
    value = 0.0
