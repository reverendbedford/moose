...
{
  const auto nelem = _batch_index_generator.getBatchIndex();
  const auto nqp = _batch_index_generator.nQp();
  ...
  for (auto & [y, dy] : _retrieved_parameter_derivatives)
    for (auto & [p, target] : dy)
      target = neml2::math::jacrev(_out[y], model().get_parameter(p)).to(torch::kCPU);
  ...
  for (auto & [y, dy] : _retrieved_derivatives)
    for (auto & [x, target] : dy)
    {
      ...
      if (source.defined())
        target = source.to(torch::kCPU).batch_expand({neml2::Size(nelem), neml2::Size(nqp)});
    }
